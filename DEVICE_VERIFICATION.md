...
| Navigation — `createNavigator()` (`mithril-lynx/navigation`, added 2026-09-10) | **Confirmed WORKING on-device 2026-09-10** | Scaffolded `create-mithril-lynx`'s "Basic Activity" template (Main → Details, via `nav.push()`/back button/`nav.pop()`) and ran it on the Galaxy A07. Tapping "Next" pushed Details with the passed data showing correctly; tapping the back arrow popped back to Main — the resulting screenshot was **byte-identical** to the one taken before navigating away, confirming a clean, complete round trip with zero console errors. No new PAPI surface is involved (built entirely on `shim.redraw()`), which is also why this needed no fix, unlike gestures/lists/renderer mode above. |
| Custom fonts — `@font-face` (README's "Custom fonts" section, added 2026-09-10) | **Confirmed FIXED and WORKING on-device 2026-09-10** | Built as part of the `indicadores-app` sample. A `.woff2` `@font-face` compiled into a valid embedded data URI with zero build/runtime errors, yet was never applied — confirmed with a maximally-distinctive test font (a cursive/marker face swapped for the default sans-serif) producing zero visual change. Re-pointing the same rule at a `.ttf` of the same font (no other change) worked immediately, everywhere in the tree. Also found: `font-family` set on `:root` doesn't cascade without `pluginLynxConfig({ enableCSSInheritance: true })` — off by default. |

One proven baseline, one reconfirmed-in-passing (data-channel mode wasn't re-tested directly this round, still believed fine), eight confirmed working on-device (renderer mode, style-removal, refs `invoke()`, list Tier 2, gestures, background `createRef()`, navigation), two remaining "probably fine" items (List Tier 1, cross-thread function registry) that ride entirely on already-proven PAPI surface and were deprioritized rather than skipped by oversight.
...
### Custom fonts: resolution (2026-09-10)

Built inside `indicadores-app` (a real, separate app consuming `mithril-lynx`, not a Demo A/B addition), while adding an Ubuntu Mono typeface at the user's request:

1. **`lynx.addFont()` tried first** (matching `@lynx-js/type-element-api`'s ambient types, background-thread only): the callback (undocumented as required, `@lynx-js/types` marks it optional) threw `"The second argument must be function type"` when omitted; passing one fixed that, and the callback did fire — but the registered `font-family` never visibly applied to any main-thread text, no matter where declared.
2. **Found the actual recommended pattern by searching `lynx-family/lynx-examples` directly** (`gh search code`): `examples/text/src/font_face` uses a plain CSS `@font-face`, not `lynx.addFont()` at all — that JS API turns out to be for *dynamic* loading after mount only (`examples/text/src/custom_font`, which pairs it with `componentDidMount` + `setState` to force a re-render once its callback confirms the font is ready). Switched to `@font-face` — it compiled into a valid data URI with zero errors, but still didn't visibly apply anywhere, even after also enabling `enableCSSInheritance` (a separate, real gap: `font-family` on `:root` doesn't cascade to descendants without it) and even with a maximally-distinctive test font (a cursive/marker face — if it had applied at all, it would have been unmistakable).
3. **Root cause, found by the user**: the font file must be **`.ttf`**, not `.woff2`. Both official examples (`font_face` and `custom_font`) use `.ttf` exclusively; this had gone unnoticed because Fontsource-distributed npm packages (the initial font source) only ship woff/woff2. Re-pointing the identical `@font-face` rule at a `.ttf` of the same font (fetched directly from `google/fonts`, since Fontsource has no `.ttf`) — no other change — worked immediately, on every element in the tree.

Net effect: `font-family: "Ubuntu Mono"` on `:root`, cascading via `enableCSSInheritance: true`, renders correctly everywhere in `indicadores-app` on the Galaxy A07, in both the dark and light theme.
