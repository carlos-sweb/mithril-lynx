...
One proven baseline, one reconfirmed-in-passing (data-channel mode wasn't re-tested directly this round, still believed fine), five confirmed working (renderer mode, style-removal, refs `invoke()`, list Tier 2), one confirmed broken as currently implemented (gestures), one untouched (background `createRef()`).

### What would unblock gestures, the one remaining confirmed-broken item

Gestures fail *silently* — no exception, confirmed with `lynx-devtool`'s live console open (zero output on a real swipe). List Tier 2's own silent failure turned out to have a discoverable, specific native error one layer deeper (the `update-list-info` fix), reachable once the right call was made — worth trying the analogous move for gestures: check whether there's an equivalent "the real trigger is one step past what the type signature suggests" for `__SetGestureDetector`/`__CreateGestureDetector`, rather than assuming a Worklet-shaped callback is required just because nothing fires. Needs either a minimal Worklet-shaped callback wrapper attempt, or confirmed documentation of what the callback slot actually requires.
