...
| Refs — `element.js`'s `invoke()` (Phase 5) | **Inconclusive on-device 2026-09-09** | Tapped the test button (`invoke("boundingClientRect", {})`) — no crash, but no visible change either, and the promise never appeared to settle (neither `.then` nor `.catch` fired, based on the on-screen status text never updating). Most likely explanation: `"boundingClientRect"` isn't a real native UI method name (it was a guess in the demo, not sourced from anywhere), so native silently never calls the callback — **not yet evidence that `invoke()`'s wiring itself is broken**, just that this specific method name wasn't a real one. Needs a real method name (from `lynx-devtool`'s element inspector or Lynx docs) to actually test the mechanism. |
...
| Gestures — `createGesture()` (Phase 7) | **Confirmed BROKEN as-implemented, on-device 2026-09-09** | Registered a `PAN` gesture on a real element (no crash — `__SetAttribute("has-react-gesture", ...)` and `__SetGestureDetector(...)` both accepted the call silently), then drove a real `adb shell input swipe` across it. Callback never fired (`panLog` stayed `"no pan yet"`). This is exactly the failure mode the README predicted: the plain-function callback assumption is very likely wrong — native probably expects a Worklet-shaped object and silently no-ops otherwise, rather than throwing. **Needs `lynx-devtool` console access to confirm** (no exception was visible without it) before attempting a fix — don't guess further blind. |
...
One proven baseline, one reconfirmed-in-passing (data-channel mode wasn't re-tested directly this round, still believed fine), three confirmed working (renderer mode, style-removal), two confirmed broken as currently implemented (gestures, list Tier 2), one inconclusive pending a better test method name (refs `invoke()`), one untouched (background `createRef()`).

### What would unblock the two confirmed-broken items fastest

Both gestures and list Tier 2 failed *silently* — no exception surfaced anywhere visible without console access. `lynx-devtool` was connected earlier in this session but disconnected partway through (cause not diagnosed — possibly the repeated `am force-stop`/cold-relaunch cycles used to force fresh bundle loads dropped its debug-bridge connection). Reconnecting it and re-running the gesture swipe / list scroll with its console and element-tree inspector open is the highest-value next step — guessing further at the native contract from screenshots alone has hit diminishing returns.
